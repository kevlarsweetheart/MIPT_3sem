 ...
 * 
 * Да, на маленьких циклах время перемножения фактически меньше времени создания нитей, переключения контекста и прочих накладных расходов.
 * Сделайте ф-и генерации пары больших матриц (1000 * 1000) и продемонстрируйте boost от параллелизма.
 * Давайте вы это сделаете и завтра на семинаре покажете, т.к. у меня по коду замечаний нет.
 * 
 * Не всегда получится использовать omp, поэтому хотя бы продумайте, как бы оно выглядело с использованием posix threads.
 */
